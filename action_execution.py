...
import rospy
from mdp_tg.msg import action, confirmation, cell_pose, raw_pose, control
...
def action_execution():
    ...
    control_pub = rospy.Publisher('control_cmds', control, queue_size=100)
    ###### subscribe to
    ...
    while not rospy.is_shutdown():
        if next_action[0] == t:
            ...
            start_pose = raw_pose[:]
            goal_pose = Find_Goal(start_pose, grid, action_name)
...
def Find_Goal(start_pose, grid, action_name):
...
def  Find_Control(raw_pose, goal_pose):
    ...
    orientation = atan(g_y-s_y, g_x-s_x)
    if ((distance((s_x,s_y), (g_x,g_y)) > 0.1) and (abs(s_theta, orientation) > 0.1*PI)):
        print 'Turn to face the goal'
        if ((0 < s_theta-orientation < 1.0*PI) or (-2.0*PI < s_theta-orientation < -1.0*PI)):
            angular_V = -ANGULAR_V 
        else:
            angular_V = ANGULAR_V 
    elif ((distance((s_x,s_y), (g_x,g_y)) > 0.1) and (abs(s_theta, orientation) < 0.1*PI)):
        print 'Forward to the goal'
        linear_V = LINEAR_V
    elif ((distance((s_x,s_y), (g_x,g_y)) < 0.1) and (abs(s_theta, g_theta) > 0.1*PI))::
        print 'Turn to right orientation'
        if ((0 < s_theta-g_theta < 1.0*PI) or (-2.0*PI < s_theta-g_theta < -1.0*PI)):
            angular_V = -ANGULAR_V 
        else:
            angular_V = ANGULAR_V 
